...
func init() {
	android.RegisterModuleType("nyanpasu_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = nyanpasuExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = nyanpasuExpandVariables(ctx, dep_file)
	...
	cmd := nyanpasuExpandVariables(ctx, String(g.properties.Cmd))
