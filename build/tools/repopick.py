...
if __name__ == '__main__':
    # Default to Nyanpasu Gerrit
    default_gerrit = 'https://review.reallysnow.moe'

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from Nyanpasu's Gerrit instance (or any gerrit instance of your choosing)
    ...
    for project in projects:
        name = ("Project-Nyanpasu/")+project.get('name')
    ...
    for item in mergables:
        ...
        # Try fetching from Nyanpasu Org first if using default gerrit
        if args.gerrit == default_gerrit:
            if args.verbose:
                print('Trying to fetch the change from Nyanpasu Org')

            if args.pull:
                cmd = ['git pull --no-edit nyanpasu', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch nyanpasu', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or nyanpasu org failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
                    print('Fetching from Nyanpasu Org didn\'t work, trying to fetch the change from Gerrit')
                else:
